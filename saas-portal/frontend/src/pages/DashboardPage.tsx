...
export const DashboardPage: React.FC<DashboardPageProps> = ({ user, onLogout, onUserUpdated }) => {
  ...
  const handleGridMonitor = () => {
    if (selectedDevices.size === 0) return;
    const targets = Array.from(selectedDevices).map(id => {
      const dev = devices.find(d => d.session_id === id);
      return dev ? `${id}:${encodeURIComponent(dev.name)}:${dev.direct_ip || ''}:${dev.direct_port || ''}` : id;
    }).join(',');
    window.open(`http://localhost:9000?monitoring=true&targets=${targets}`, '_blank');
  };
  ...
  const fetchDevices = async () => {
    ...
    try {
      ...
      setSelectedDevices(new Set()); // Reset selections on refresh
    } catch (err) {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white flex items-center space-x-2">
              ...
              <span className="text-[10px] uppercase tracking-widest bg-amber-500/20 text-amber-300 font-bold px-2 py-0.5 rounded-full border border-amber-500/30 flex items-center space-x-1.5">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {selectedDevices.size > 0 && (
              <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl animate-in slide-in-from-top duration-200">
                <div className="flex items-center space-x-3">
                  <div className="p-2.5 bg-blue-600/10 rounded-xl text-blue-400 border border-blue-500/20">
                    <Network className="w-5 h-5 animate-pulse" />
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-slate-100">{selectedDevices.size} Device(s) Selected</h4>
                    <p className="text-xs text-slate-400">Launch side-by-side low-latency grid view monitoring session</p>
                  </div>
                </div>
                <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
                  ...
                  <button
                    onClick={handleGridMonitor}
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-semibold shadow-lg shadow-blue-500/20 flex items-center space-x-1.5 transition-all"
                  >
                    <span>Monitor in Grid View</span>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-2.5">
                        {device.is_online && (
                          <input
                            type="checkbox"
                            checked={selectedDevices.has(device.session_id)}
                            onChange={() => handleToggleSelect(device.session_id)}
                            className="w-4 h-4 rounded border-slate-800 bg-slate-950 text-blue-600 focus:ring-blue-500/30 cursor-pointer"
                          />
                        )}
